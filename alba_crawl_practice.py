import os  #os 터미널 창 지우는 것
import csv #csv(comma seperated value)  - 컴마로 되어있어서 엑셀로 출력할 수도 있고,,
import requests #확장프로그램
from bs4 import BeautifulSoup #확장프로그램

os.system("clear") #터미널 창 없어주는 것
...
brands = main.find_all("li", {"class": "impact"})  #find_all은 모든 li를 가져오는 것

for brand in brands:
    link = brand.find("a", {"class": "goodsBox-info"})
    name = brand.find("span", {"class": "company"})
    if link and name:
        link = link["href"]   #link를 가져올때 href
        name = name.text
        print("Getting " + name + " info...")
        company = {'name': name, 'jobs': []}
        jobs_request = requests.get(link)  #위에 것과 같은 형식 for 문 안에 있는 것만 다름
        jobs_soup = BeautifulSoup(jobs_request.text, "html.parser")
        tbody = jobs_soup.find("div", {"id": "NormalInfo"}).find("tbody") #tbody = tablebody = 표
